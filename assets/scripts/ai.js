...
sendQueryButton.addEventListener("click", async () => {
  ...
  const apiUrl = `https://api-faa-skuarta2.vercel.app/faa/chatai?text=${encodeURIComponent(userInput)}&model=${encodeURIComponent(selectedModel)}`;

  try {
    const response = await fetch(apiUrl);
    const result = await response.json();

    if (result.status && result.result && result.result.response && result.result.response.response) {
      aiResponseDiv.textContent = result.result.response.response;
      copyResponseButton.style.display = "inline-block";
    } else {
      aiResponseDiv.textContent = "Error: " + (result.message || "Gagal mendapatkan respons dari AI. Coba lagi nanti.");
      copyResponseButton.style.display = "none";
    }
  } catch (error) {
